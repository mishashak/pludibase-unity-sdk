...
**이 세 개로 소셜 로그인, 이벤트, 결제 검증, 플레이어 병합까지 다 됩니다.** 결제 검증 때문에
키를 다시 받으실 필요는 없습니다. 결제에 필요한 건 스코프가 아니라 게임별 스토어 자격증명
(Play 서비스 계정 키 등)이고, 그건 저희가 백엔드에 등록합니다.

하나만 예외입니다. **오프라인 기록**(비행기 모드에서 쌓아뒀다가 나중에 올리는 이벤트)의
발생 시각을 살리려면 `write:continuityRequests` 가 추가로 필요합니다. 이 스코프가 없으면
403이 아니라 **조용히 무시되고 동기화한 시각으로 저장**되니, 쓰실 거면 미리 말씀해 주세요.
스코프는 키를 다시 발급하지 않고 추가됩니다. 쓰시던 키 그대로 두시면 됩니다.
...
13. ⚠️ **오프라인 기록(continuity)은 별도 스코프가 필요합니다.**
    ...
    **오류가 나지 않고 서버가 그 타임스탬프를 조용히 무시합니다.**
    ...
    에러가 안 나서 눈치채기 어렵습니다. 오프라인 대응이 필요하면 말씀해 주세요.
    **키를 다시 발급하지 않고 스코프만 추가됩니다.** 쓰시던 키 그대로 두시면 됩니다.
